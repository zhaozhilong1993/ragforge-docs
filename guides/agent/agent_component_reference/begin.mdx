...
No. Files uploaded to an agent as input are not stored in a knowledge base and hence will not be processed using RAGForge's built-in OCR, DLR or TSR models, or chunked using RAGForge's built-in chunk methods. 
